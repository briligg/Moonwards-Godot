...
func camera_ready(force : bool = false) -> void:
	...
	if not active or force:
		...
		camera.current = true
		if active:
			Log.hint(self, "sync camera position with old camera")
			camera.global_transform = camera_ready_oldcamera.global_transform
		Log.hint(self, "added fly camera to scene, index %s" % camera_used)

func on_scene_change() -> void:
	Log.hint(self, "on_scene_change")
...
func print_active_cameras() -> void:
	...
	for p in cameras:
		Log.hint(self, "%s(%s)" % [p, root.get_node(p).current])

func set_active_camera() -> void:
	Log.hint(self, "set camera to local player: %s" % gamestate.local_id)
...
func set_3fps(enable : bool, value : int = 3) -> void:
	if enable:
		Log.hint(self, "debug set FPS to %s" % round(value))
		Engine.target_fps = round(value)
	else:
		Log.hint(self, "debug set FPS to 0")
		Engine.target_fps = 0
...
func e_collision_shapes(enable : bool = true):
	...
	Log.hint(self, "e_collision_shape(enable=%s), found : %s" % [enable, cs_objects.size()])
	for p in cs_objects:
...
func set_lod_manager(enable : bool) -> void:
	...
	if slm == null:
		...
		Log.hint(self, "Look for existing TreeManager")
		for p in utils.get_nodes_type(root, "Node", true):
			...
			if obj.script and obj.get("id") and obj.id == "TreeManager":
				...
				Log.hint(self, "found TreeManager at %s" % p)
				break
		if enable == null:
			#just find if there is lod manager in the tree
			Log.hint(self, "end search for LodManager")
			return
	...
	if not enable:
		if slm:
			...
			Log.hint(self, "set_lod_manager, attempt to disable notexisting tree manager")
		return #nothing to do here
		
	if slm == null:
		#create/add proper node
		Log.hint(self, "Load TreeManager")
...
func features_list(enabled_only : bool = true) -> void:
	...
	if enabled_only:
		Log.hint(self, "OS:: print only enabled features")
	
	for f in features:
		if enabled_only:
			if OS.has_feature(f.opt):
				Log.hint(self, "OS::%s has %s" % [f.opt, OS.has_feature(f.opt)])
		else:
			Log.hint(self, "OS::%s has %s" % [f.opt, OS.has_feature(f.opt)])
	

func print_current_players() -> void:
	Log.hint(self, "gamestate players")
	print(gamestate.players)
	for p in gamestate.players.keys():
		Log.hint(self, "player %s" % gamestate.players[p])
		Log.hint(self, "obj at %s" % gamestate.players[p].obj.get_path())

func print_groups() -> void:
	#get_nodes_in_group("LODElement)
	Log.hint(self, "List of nodes in LODElement group")
	for obj in get_tree().get_nodes_in_group("LODElement"):
		print(obj.get_path())
	Log.hint(self, "List of nodes in wall group")
...
func mouse_toggle() -> void:
	match Input.get_mouse_mode():
		Input.MOUSE_MODE_VISIBLE:
			...
			Log.hint(self, "set cursor to captured")
		Input.MOUSE_MODE_CAPTURED:
			Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
			Log.hint(self, "set cursor to visible")
		_:
