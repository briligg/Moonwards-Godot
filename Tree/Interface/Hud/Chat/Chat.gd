...
#This is present for later implementations.
var messages_available : bool = false
...
#When I am too small vertically, keep tabs hidden.
func _on_Chat_resized():
	#This gets called too soon before the onready nodes
	#are assigned. Exit out if _tabs_node is null.
	if _tabs_node == null || messages_available == false:
		return
	
	if rect_size.y < 150 :
		_tabs_node.tabs_visible = false
	else :
		_tabs_node.tabs_visible = true
